...
| 품질 관리 / 자동화 | Vitest, Testing Library, ESLint, GitHub Actions | 주요 사용자 흐름 테스트와 코드 품질 검사, CI 자동 검증 |
...
PR과 `main` 브랜치 기준으로 기본적인 코드 품질, 테스트 통과 여부, 빌드 가능 여부를 확인하도록 구성했습니다.

## 테스트

`Vitest`와 `Testing Library`를 사용해 `HabitAddForm`의 주요 흐름을 테스트했습니다.

- 습관 추가 성공
- 요청 실패 시 실패 처리
- 입력값이 비어 있을 때 제출 차단

이를 통해 폼의 기대 동작을 테스트로 고정하고, 이후 리팩토링 시에도 동일한 동작을 검증할 수 있도록 했습니다.

## 앞으로 보완하고 싶은 점

- 폼 외 다른 사용자 흐름까지 테스트 범위 확장
- 차트/통계 표현 확장
